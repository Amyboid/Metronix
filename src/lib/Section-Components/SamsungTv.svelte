<section class="homepage-section gradient-bg-section relative">
	<div
		class="mt-14 mb-12 flex h-full w-full flex-col items-center justify-between gap-4 sm:mt-12 sm:mb-12 sm:gap-4"
	>
		<div
			class="samsung-tv-img relative z-10 w-[300px] items-center sm:flex sm:w-[420px] md:w-[400px]"
		>
			<enhanced:img class="z-30" src="$lib/assets/landing-page/samsung-tv.png" alt="samsung-tv" loading="lazy" />
		</div>
		<div class="z-10 flex flex-col items-center">
			<h1 class="z-30 text-center text-3xl font-semibold tracking-wider">Samsung tv</h1>
			<div class="z-30 mt-1 flex items-center text-sm sm:text-lg">
				<span class="mr-2 text-center">Endless Viewing Pleasure.</span>
			</div>
			<a data-sveltekit-preload-data="off" data-sveltekit-reload href="/products/tv"> 
				<button
				class="visit-btn-dark mt-6 flex cursor-pointer items-center justify-center rounded-lg p-2 tracking-wider sm:mt-10 sm:pr-4 sm:pl-4 md:tracking-widest"
				>Visit Now</button
				>
			</a>
		</div>
	</div>

	<h1 class="absolute top-6 left-6 text-center text-xl font-extrabold">SAMSUNG</h1>
</section>

<style>
	@import url('./styles.css');
	.visit-btn-dark {
		border: 1px solid black;
		color: black;
	}
	.gradient-bg-section {
		background: linear-gradient(59deg, rgba(255, 207, 48, 1) 0%, rgba(253, 45, 225, 1) 100%);
	}
</style>
