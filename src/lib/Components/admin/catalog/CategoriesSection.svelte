<script lang="ts">
	...
	async function addItem() {
		...
		try {
			...
			if (addPending) {
				addUploading = true;
				const r = await uploadToIK(addPending.file, 'assets/hero-banner');
	...
	// Close edit row after a SUCCESSFUL save — does NOT delete anything from IK
	function closeEdit(slug: string) {
		const { [slug]: _, ...rest } = editing;
		editing = rest; rowError = { ...rowError, [slug]: '' };
	}

	// Cancel edit (user clicked ✕) — revokes object URL, no IK cleanup needed
	...
	async function saveEdit(item: Category) {
		...
		let finalPath   = ed.bannerPath;
		let finalFileId = ed.bannerFileId;
		let uploadedFileId: string | null = null;

		if (ed.pending) {
			try {
				const r = await uploadToIK(ed.pending.file, 'assets/hero-banner');
				uploadedFileId = r.fileId;
		...
		try {
			const res = await fetch('/api/admin/catalog', {
				...
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					section: 'category', slug: item.slug,
	...
	// ── Delete modal ──────────────────────────────────────────────────────────
	async function startDelete(item: Category) {
		deleteTarget = item; deleteChecking = true; deleteConfirming = false;
		try {
			const res  = await fetch(`/api/admin/catalog?section=product-count&entity=category&slug=${item.slug}`);
			const data = await res.json();
	...
	<div class="fixed inset-0 z-[200] bg-black/35 backdrop-blur-[2px] flex items-center justify-center p-6" on:click|self={closeDeleteModal} on:keydown={(e) => { if (e.key === 'Escape') closeDeleteModal(); }} role="dialog" tabindex="-1" aria-modal="true">
		<div class="bg-neutral border border-subtle rounded-xl p-6 w-full max-w-[480px] flex flex-col gap-4 shadow-[0_8px_32px_rgba(0,0,0,0.12)]">
			...
			{:else}
				...
				{#if deleteProductCount > 0}
					<div class="flex gap-2.5 items-start bg-[#fff7ed] border border-[#fed7aa] rounded-lg p-3.5 text-[13px] text-[#92400e]">
						...
						<span>
							...
							{deleteProductCount !== 1 ? 'are' : 'is'} linked to this category.
					...
					<div class="flex gap-2 justify-end flex-wrap">
						...
						<button class="font-inter text-[13px] font-semibold py-[7px] px-4 rounded-[7px] border border-subtle bg-surface text-copy cursor-pointer transition-colors hover:bg-canvas"   on:click={() => confirmDelete(false)}>Delete category only</button>
				...
				{:else}
					<p class="text-sm text-copy m-0">This category has no linked products and will be permanently deleted.</p>
					<div class="flex gap-2 justify-end flex-wrap">
						<button class="font-inter text-[13px] font-medium py-[7px] px-4 rounded-[7px] border border-subtle bg-transparent text-copy cursor-pointer transition-colors hover:bg-surface" on:click={closeDeleteModal}>Cancel</button>
...
<div class="flex flex-col gap-5">
	<div class="flex items-start justify-between gap-3">
		<div>
			<h2 class="text-base font-bold text-gray-900 mb-0.5">Categories</h2>
			<p class="text-[13px] text-copy-light m-0">Manage product categories, banner images and messages.</p>
		</div>
	...
	{#if showAdd}
		<div class="bg-surface border border-subtle rounded-[10px] px-5 py-[18px] flex flex-col gap-3.5">
			<h3 class="text-sm font-bold text-gray-900 m-0">New Category</h3>
			<div class="flex gap-3.5 items-start flex-wrap">
				<div class="w-[200px] h-[90px] shrink-0 border border-subtle rounded-lg overflow-hidden bg-canvas flex items-center justify-center">
					{#if addPending}
						<img class="w-full h-full object-cover" src={addPending.previewUrl} alt="Preview" />
					{:else}
				...
				<div class="flex flex-col gap-2 justify-center">
					<label class="inline-flex items-center gap-[5px] font-inter text-xs font-semibold py-1.5 px-3 rounded-md border border-subtle bg-neutral text-copy cursor-pointer whitespace-nowrap transition-colors" class:uploading={addUploading}>
						{#if addUploading}<span class="inline-block w-2.5 h-2.5 border-2 border-subtle border-t-primary rounded-full animate-[spin_0.7s_linear_infinite]"></span> Uploading…
						{:else}
			...
			<div class="flex gap-3 flex-wrap two-col">
				<label class="flex flex-col gap-[5px]">
					...
					<input class="font-inter text-[13px] py-[7px] px-2.5 border border-subtle rounded-md bg-neutral text-gray-900 outline-none transition-colors w-full box-border focus:border-primary" bind:value={addName} on:input={() => { addSlug = nameToSlug(addName); }} placeholder="e.g. Consumer Electronics" />
				</label>
				<label class="flex flex-col gap-[5px]">
			...
			<label class="flex flex-col gap-[5px]">
				...
				<input class="font-inter text-[13px] py-[7px] px-2.5 border border-subtle rounded-md bg-neutral text-gray-900 outline-none transition-colors w-full box-border focus:border-primary" bind:value={addBannerMsg} placeholder="Explore our latest collection…" />
			</label>
	...
	<!-- ── Table ── -->
	{#if loading}
		<div class="flex flex-col gap-2 w-full">{#each Array(5) as _}<div class="w-full h-12 rounded-lg shimmer"></div>{/each}</div>
	{:else if items.length === 0}
		<div class="py-8 text-center text-sm text-copy-light border border-dashed border-subtle rounded-[10px]">No categories yet. Add one above.</div>
	{:else}
		<div class="border border-subtle rounded-[10px] overflow-hidden overflow-x-auto">
			<table class="w-full border-collapse text-[13px]">
				<thead>
					<tr>
						<th class="py-2.5 px-3.5 text-left text-[11px] font-bold uppercase tracking-[0.06em] text-copy-light border-b border-subtle whitespace-nowrap w-20">Banner</th>
						...
						<th class="py-2.5 px-3.5 text-left text-[11px] font-bold uppercase tracking-[0.06em] text-copy-light border-b border-subtle whitespace-nowrap">Banner Message</th>
				...
				<tbody>
					{#each items as item (item.slug)}
						...
						{@const displayPath = ed ? (ed.pending?.previewUrl ?? ed.bannerPath) : item.bannerPath}
						<tr class:editing-row={!!ed}>
							<td class="w-20 py-2 px-3.5 text-copy border-b border-subtle">
								{#if displayPath}
									<div class="w-16 h-9 rounded-[5px] overflow-hidden bg-canvas border border-subtle"><img class="w-full h-full object-cover" src={displayPath.startsWith('blob:') ? displayPath : (thumb(displayPath) ?? '')} alt={item.name} /></div>
								{:else}
							...
							{#if ed}
								<td class="py-2 px-3.5 text-copy border-b border-subtle"><input class="font-inter text-[13px] py-[5px] px-2 border border-subtle rounded-md bg-neutral text-gray-900 outline-none transition-colors w-full box-border min-w-[90px] focus:border-primary" bind:value={ed.name} /></td>
								<td class="py-2 px-3.5 text-copy border-b border-subtle">
									<div class="flex items-center gap-1.5 flex-wrap">
										<label class="inline-flex items-center gap-[5px] font-inter text-[11px] font-semibold py-1 px-2 rounded-md border border-subtle bg-neutral text-copy cursor-pointer whitespace-nowrap transition-colors" class:uploading={ed.uploading}>
											{#if ed.uploading}<span class="inline-block w-2.5 h-2.5 border-2 border-subtle border-t-primary rounded-full animate-[spin_0.7s_linear_infinite]"></span>
											{:else}
						...
						{#if rowError[item.slug]}
							<tr class="error-row"><td colspan="5"><span class="text-xs text-danger">{rowError[item.slug]}</span></td></tr>
						{/if}
					{/each}
				</tbody>
			</table>
