<script lang="ts">
		...
		try {
			const res = await fetch('/api/admin/catalog', {
				...
				body: JSON.stringify({
					...
					bannerPath:   addPending?.filePath ?? null,
					bannerFileId: addPending?.fileId   ?? null,
				}),
			});
	...
	// Cancel edit (user clicked ✕) — deletes any pending orphan upload from IK
			...
			{:else}
				...
				{#if deleteProductCount > 0}
					...
					<div class="modal-actions">
						...
						<button class="btn-modal-soft"   on:click={() => confirmDelete(false)}>Delete category only</button>
				...
				{:else}
					<p class="modal-body">This category has no linked products and will be permanently deleted.</p>
					<div class="modal-actions">
						<button class="btn-modal-cancel" on:click={closeDeleteModal}>Cancel</button>
...
<div class="section">
	<div class="section-header">
		<div>
			...
			<p class="section-sub">Manage product categories, banner images and messages.</p>
		</div>
		...
		<div class="add-form">
			...
			<div class="form-row two-col">
				<label class="field">
					...
					<input class="input" bind:value={addName} on:input={() => { addSlug = nameToSlug(addName); }} placeholder="e.g. Consumer Electronics" />
				</label>
				<label class="field">
			...
			<label class="field">
				...
				<input class="input" bind:value={addBannerMsg} placeholder="Explore our latest collection…" />
			</label>
...
<style>
	...
	.image-preview-box {
		...
		border: 1px solid var(--color-subtle); border-radius: 8px; overflow: hidden;
		background: var(--color-canvas); display: flex; align-items: center; justify-content: center;
	}
