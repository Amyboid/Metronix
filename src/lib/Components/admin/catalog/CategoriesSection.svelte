<script lang="ts">
	...
	function _bulkDelete() {
		...
		if (!confirm(`Delete ${selectedIds.length} categories? This cannot be undone.`)) return;
		(async () => {
			try {
	...
	let panelItem:   Category | null = $state(null);
	let formName     = $state('');
	let formSlug     = $state('');
	...
	// ── Delete modal ──────────────────────────────────────────────────────────
	let deleteTarget:       Category | null = $state(null);
		...
		try {
			...
			if (panelMode === 'add') {
				const res = await fetch('/api/admin/catalog', {
					...
					body: JSON.stringify({
						section: 'category', slug: formSlug.trim(), name: formName.trim(),
			...
			if (!res.ok) await handleApiError(res);
				...
				const res = await fetch('/api/admin/catalog', {
					...
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
						section: 'category', slug: panelItem!.slug,
...
<DeleteConfirmModal
	...
	message="This category and its banner will be permanently deleted."
...
<SidePanel
	open={panelMode !== null}
	title={panelMode === 'add' ? 'New Category' : 'Edit Category'}
	...
	<div class="flex flex-col gap-4">
		...
		<div class="flex gap-3 flex-wrap">
			<label class="flex flex-col gap-[5px] flex-1 min-w-[200px]">
				...
				<input class="font-inter text-[13px] py-[7px] px-2.5 border border-subtle rounded-md bg-neutral text-gray-900 outline-none transition-colors w-full box-border focus:border-primary" bind:value={formName} oninput={() => { formSlug = nameToSlug(formName); }} placeholder="e.g. Consumer Electronics" />
			</label>
			<label class="flex flex-col gap-[5px] flex-1 min-w-[200px]">
		...
		<label class="flex flex-col gap-[5px]">
			...
			<input class="font-inter text-[13px] py-[7px] px-2.5 border border-subtle rounded-md bg-neutral text-gray-900 outline-none transition-colors w-full box-border focus:border-primary" bind:value={formBannerMsg} placeholder="Explore our latest collection…" />
		</label>
...
<section class="flex flex-col gap-5">
	...
	>
		{#each items as item: Category (item.slug)}
			...
			<tr class="hover:bg-surface/50 cursor-pointer transition-colors" onclick={() => handleRowClick(item.slug)}>
				<td class="border-subtle border px-3 py-2" onclick={(e) => e.stopPropagation()}>
					<input type="checkbox" checked={isSelected} onchange={() => toggleSelect(item.slug)} class="accent-primary cursor-pointer" />
				</td>
				...
				<td class="border-subtle border px-3 py-2 text-copy-light">{item.bannerMsg ?? '—'}</td>
			</tr>
		{/each}
	</DataTable>
	{#if hasMore}
